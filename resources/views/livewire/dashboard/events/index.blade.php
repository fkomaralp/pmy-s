...
{{--           class="w-1/4 flex items-center justify-center px-4 py-2 border border-transparent rounded-md shadow-sm text-base font-medium text-white bg-positive-500 hover:bg-positive-600">Create--}}
{{--            Event</a>--}}
{{--    </div>--}}
</x-slot>

<div class="w-full mx-auto py-6 sm:px-8 lg:px-8" x-data="{ confirmModal: false, deleting: false }" x-model="delete_id">

    <div class="flex mb-6 flex-row space-x-8 bg-white rounded-md py-5 px-5">
        <div class="flex-none my-auto">
            <a href="{{route("dashboard.events.create")}}"
               class=" px-4 py-2 border border-transparent rounded-md shadow-sm text-base font-medium text-white bg-pgreen-800 hover:bg-pgreen-900">Create
                Event</a>
        </div>
        <div class="flex-none">
            <x-jet-input id="search" type="text" class="mt-1 block w-full" wire:model="search"
                         placeholder="Search by title, date, country, city"
                         autofocus/>
        </div>
    </div>
    ...
    <table class="w-full">
        ...
        <tr>
            ...
            <th scope="col"
                class="py-3 px-6 text-xs font-semibold font-medium tracking-wider text-left text-gray-700 uppercase dark:text-gray-400">
                Edit
            </th>
        </tr>
        </thead>
        ...
        @foreach($events as $event)
            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                ...
                @endif
                <td class="py-4 px-6 text-sm font-medium {{($event->protected) ? "text-green-500" : "text-red-500"}} text-gray-900 whitespace-nowrap dark:text-white">
                     {{$event->event_code}}
                </td>
...
{{--                    {{$event->city->name}}--}}
                ...
                <td class="py-4 px-6 text-sm text-gray-500 whitespace-nowrap dark:text-gray-400">
                    <div class="inline-flex rounded-md shadow-sm" role="group">
                        ...
                        <button type="button" @click.prevent="confirmModal=true; this.delete_id = {{$event->id}}"
        ...
        <div class="rounded bg-white w-2/7 object-none object-center -mt-11 border border-gray-400"
            ...
            <div class="relative px-4 w-full max-w-md h-full md:h-auto">
                ...
                <div class="p-6 pt-0 text-center">
                    ...
                    <button type="button" @click="deleting=true; $wire.delete(delete_id)"
